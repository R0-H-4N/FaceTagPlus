...
class SceneRecognition:
    def __init__(self, scenes_csv_path=None):
        ...
        model_name = 'RN101'  # ViT-L-14, ViT-B-16, etc.
        ...
        self.tokenized_text = self.tokenizer(
            self.scene_prompts).to(self.device)
        with torch.no_grad():
            self.text_features = self.model.encode_text(self.tokenized_text)
    ...
    def classify_scene(self, image):
        with torch.no_grad():
            ...
            # Compute similarities with pre-computed text features
